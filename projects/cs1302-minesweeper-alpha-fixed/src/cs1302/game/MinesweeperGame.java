...
public class MinesweeperGame {
    ...
    public void promptUser() {
        ...
        if (!(command.equals(""))) { // if user does not just hit enter
    ...
    public boolean isWon() {
        ...
        // checks if all squares without mines are revealed
        for (int i = 0; i < mineGrid.length; i ++) {
            for (int j = 0; j < mineGrid[i].length; j ++) {
                if ((mineGrid[i][j] == false) && !(Character.isDigit(gameGrid[i][j].charAt(0)))) {
                    return false;
                } // if
            } // for
        } // for
